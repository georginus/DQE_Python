...
def params(max_dict_elements=26, max_dict_value=100, random_dict_num=10):
    params_dict = {}
    params_dict.update(max_dict_elements=max_dict_elements, max_dict_value=max_dict_value,
                       random_dict_num=random_dict_num)
    return params_dict
...
filtered_params_dict = {k: v for (k, v) in params_dict.items() if
                        k in inspect.getfullargspec(define_dict)[0]}

random_dict = define_dict(**filtered_params_dict)


def list_dicts(max_dict_elements, random_dict_num):
...
filtered_params_list_dict = {k: v for (k, v) in params_dict.items() if
                             k in inspect.getfullargspec(list_dicts)[0]}
list_dict = list_dicts(**filtered_params_list_dict)
print(list_dicts)
